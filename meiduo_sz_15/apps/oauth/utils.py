...
class OauthQQ(object):
    ...
    def get_auth_url(self):
        ...
        print('这是拼接好的url地址auth_url: %s' % auth_url)
    ...
    def get_access_token_by_code(self, code):
        ...
        print('这是urlopen来获取数据的值:%s' % response)
        data = response.read().decode()

        print('这是拼接的data地址: %s' % data)
    ...
    def get_openid_by_token(self, token):
        ...
        # 字典切片  str1[开始索引:结束索引:步长]  如果结束索引为负数,则倒着数
        try:
            openid_data = json.loads(data[10:-4])
        except Exception:
            raise Exception('数据获取错误')

        print('这个是用token换取的openid:%s' % openid_data)
